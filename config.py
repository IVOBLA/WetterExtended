...
# B379: Gates der globalen Assoziation (tracking/association.py).
# ASSOC_MAX_AREA_RATIO_BASE: zulaessiges Flaechenverhaeltnis im Normaltakt. Muss
# Merge (Wachstum) und Split (Schrumpfung) zulassen — 1.25^5=3.05x verwarf zuvor
# JEDE Merge-Kontur. 10.0 entspricht dem Legacy-Stage-2-Verhalten (_aratio2>=0.10).
ASSOC_MAX_AREA_RATIO_BASE: float = 10.0
ASSOC_NORMAL_FRAME_MINUTES: float = 5.0
# B402: Nominaler Radartakt in Minuten. vx/vy sind Original-Pixel PRO FRAME; um sie
# auf einen tatsaechlichen Zeitabstand umzurechnen, wird dieser Bezug benoetigt.
# Muss mit ASSOC_NORMAL_FRAME_MINUTES uebereinstimmen -- beide beschreiben denselben
# nominalen Takt (ARSO-INCA liefert alle 5 Minuten).
FRAME_INTERVAL_MIN: float = 5.0
...
LOCAL_ANALYSIS_CONFIG: dict = {
    ...
    "timeout_s":     1700,    # B480: hartes Zeitlimit; muss zu max_turns passen und unter systemd
                              # TimeoutStartSec bleiben (B517: 3600, mit 300s Marge -> max. 3300)
...
# Datenlücke: Zwischen zwei Samples lag keine Messung vor.
HYDRO_EVENT_GAP_MIN = 180
# Trockenphase: Messungen lagen vor, precip_event_active war jedoch False.
HYDRO_EVENT_DRY_GAP_MIN = 30
HYDRO_ML_MIN_TRAIN_EVENTS = 2
HYDRO_ML_MIN_VALIDATION_EVENTS = 1
HYDRO_ML_MATERIALIZE_BATCH_SIZE = 100
HYDRO_Q_HISTORY_RETENTION_DAYS = 30
HYDRO_ML_SAMPLE_INTERVAL_MIN = 15
HYDRO_ML_MAX_SAMPLES_PER_STATION_EVENT = 500
HYDRO_ML_MAX_NO_CELL_SAMPLES_PER_DAY = 24
HYDRO_ML_MAX_TRAINING_SAMPLES = 50000

HYDRO_FLOOD_ML_FEATURES = [
    "current_q_m3s",
    "current_q_ratio_threshold",
    "current_q_distance_to_threshold_m3s",
    "q_trend_delta_m3s",
    "q_trend_reference_window_min",
    "already_rising_flag",
    "catchment_area_km2",
    "upstream_catchment_count",
    "routing_tau_min",
    "contributing_cell_count",
    "current_cell_count",
    "incoming_cell_count",
    "total_rain_volume_m3",
    "total_runoff_volume_m3",
    "total_dwell_time_min",
    "max_cell_dwell_time_min",
    "effective_overlap_area_km2",
    "overlap_area_time_km2_min",
    "cell_catchment_area_km2_sum",
    "rain_rate_mm_h_max",
    "rain_rate_mm_h_mean",
    "rain_rate_mm_h_area_weighted",
