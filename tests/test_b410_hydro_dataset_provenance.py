...
def test_legacy_q_samples_are_separated(tmp_path, monkeypatch):
    ...
    live = {'sample_id':'l','sample_kind':h.SAMPLE_KIND_LIVE,'feature_schema_version':h.FEATURE_SCHEMA_VERSION,'feature_snapshot_complete':True,'target_q_delta_m3s':1,'target_missing':False,'cell_frame_hash':'x'}
...
def test_train_filter_requires_live_complete_schema():
    row = {'sample_kind':h.SAMPLE_KIND_LEGACY,'feature_schema_version':h.FEATURE_SCHEMA_VERSION,'feature_snapshot_complete':True,'target_q_delta_m3s':1,'target_missing':False}
