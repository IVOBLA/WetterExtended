...
def test_event_is_written_and_status_ok(_isolate_state):
    ...
    status = json.loads((_isolate_state / "cell_lineage_write_status.json").read_text())
    assert status["last_result"] == "ok"
    assert status["ok_count"] == 1
    assert status["error_count"] == 0


def test_resolved_path_is_absolute(_isolate_state):
...
def test_write_failure_is_recorded_not_swallowed(_isolate_state, monkeypatch):
    ...
    cl.append_lineage_event({"event_type": "cell_merge", "cell_id": "C1"})
    status_file = _isolate_state / "cell_lineage_write_status.json"
...
def test_status_counters_accumulate(_isolate_state):
    for _ in range(3):
        cl.append_lineage_event({"event_type": "cell_merge", "cell_id": "C1"})
    status = json.loads((_isolate_state / "cell_lineage_write_status.json").read_text())
