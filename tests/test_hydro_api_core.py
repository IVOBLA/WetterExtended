...
def test_hydro_api_core_status_and_geojson(monkeypatch, tmp_path):
    ...
    (gen/'station_network_index.json').write_text(json.dumps({'stations':[{'station_id':station_id,'station_name':'Pegel A','river_name':'Gail','lon':13.1,'lat':46.6,'enabled':True,'impact_eligible':True}]}), encoding='utf-8')
...
def test_hydro_station_features_hide_disabled_station_unless_requested(monkeypatch, tmp_path):
    ...
    gen = tmp_path / 'static' / 'generated'; live = tmp_path / 'live'; impact = tmp_path / 'impact'
    gen.mkdir(parents=True); live.mkdir(); impact.mkdir()
    (gen/'station_network_index.json').write_text(json.dumps({'stations':[{'station_id':'S1','station_name':'Pegel A','river_name':'Gail','lon':13.1,'lat':46.6,'enabled':True,'impact_eligible':True}]}), encoding='utf-8')
...
def test_api_hydro_stations_hides_non_eligible_public_but_keeps_admin(monkeypatch, tmp_path):
    import hydro_api
...
def test_api_hydro_stations_filters_all_non_effective_unless_requested(monkeypatch, tmp_path):
    import hydro_api
...
def test_empty_station_catchments_does_not_empty_station_endpoint(monkeypatch, tmp_path):
    ...
    assert len(hydro_api.station_features()['features']) == 0
    assert len(hydro_api.station_features(include_disabled=True)['features']) == 1
