...
def test_readiness_uses_sqlite_not_broken_jsonl(monkeypatch, tmp_path):
    ...
    row={'sample_id':'1','station_id':'s','sample_kind':h.SAMPLE_KIND_LIVE,'feature_schema_version':h.FEATURE_SCHEMA_VERSION,'feature_snapshot_complete':True,'target_missing':False,'target_q_delta_m3s':1.0,'sample_start_time':'2026-01-01T00:00:00Z','features':features}
    with h._sample_db() as con:
        con.execute('INSERT INTO labeled_samples(sample_id,station_id,sample_start_time,labeled_at,payload) VALUES(?,?,?,?,?)', ('1','s',row['sample_start_time'],row['sample_start_time'],json.dumps(row)))
