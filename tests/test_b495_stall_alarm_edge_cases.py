...
def test_finish_experiment_clears_stall_flag_on_immediate_improvement():
    state = {"quality_improvement_stalled": True, "pending": {"experiment_id": "e1"}}
    ta.HISTORY_FILE.parent.mkdir(parents=True, exist_ok=True) if not ta.HISTORY_FILE.parent.exists() else None
    ta._finish_experiment(state, "improved", {"mean_delta_km": -1.0}, "statistically_significant_improvement")
    assert "quality_improvement_stalled" not in state


def test_finish_experiment_keeps_stall_flag_on_plateau():
