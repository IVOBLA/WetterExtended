...
def api_hydro_flood_risk():
    def _load():
        ...
        # B430: Ein Cache mit falschem oder fehlendem payload_scope wird verworfen und
        # neu berechnet, statt den Endpunkt dauerhaft mit einer Exception zu blockieren.
        # Der Guard bleibt aktiv — er darf nur nie einen public-Payload durchlassen,
        # nicht die Neuberechnung verhindern.
        if isinstance(doc, dict) and doc.get("payload_scope") != "public":
            doc = None
        ...
        if hydro_flood_ml.is_flood_risk_cache_valid(doc, live=live, cells=cells or [], cell_frame_meta=cell_meta):
            return doc
...
def api_hydro_station_patch(station_id):
    ...
    if "enabled" in data:
        ...
        if data["enabled"] is True and hydro_api.apply_station_override((hydro_api._static_index()).get(sid, {}), overrides).get("impact_eligible_auto") is not True:
            return _hydro_json("not_impact_eligible", ok=False, error="Station ist nicht impact_eligible_auto", code=400)
