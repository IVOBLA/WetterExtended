...
Hydro-Impact bewertet nur **einen plausiblen Zusammenhang** zwischen Niederschlagszellen und Pegeln. Die Zuordnung ist **nicht radiusbasiert**: Der nächste Pegel ist fachlich kein gültiges Kriterium, weil Niederschlag neben einem Gewässer, in einem anderen Teileinzugsgebiet oder unterhalb der Messstelle keine belastbare Reaktion an dieser Station erklären kann. Zulässig ist nur eine Zelle, die das lokal hinterlegte **oberliegende Einzugsgebiet** einer Station schneidet; zusätzlich muss ein hydrologischer **Zeitversatz** für die mögliche Pegel-/Abflussreaktion berücksichtigt werden.

Die Anzeige ist immer vorsichtig zu verstehen: Sie ist **keine harte Kausalität**, **keine amtliche Hochwasserwarnung** und kein Ersatz für geprüfte amtliche Endwerte. Live-Hydro-Daten sind Rohdaten beziehungsweise Live-Indikatoren und können später qualitätsgesichert, korrigiert oder verworfen werden.

### Hydro-Impact: fachlich sichere Pegel-Attribution

WetterExtended erzeugt Hydro-Impact nur für Stationen mit `impact_eligible=true`. Dieses Flag bedeutet derzeit konservativ, dass explizite lokale `upstream_catchment_ids` vorhanden und gegen Basin-Geometrien validiert sind und daraus ein geometrisch vereinigtes oberliegendes Einzugsgebiet (`station_catchment`) erzeugt wurde; eine automatische gerichtete Fließtopologie aus Flowlines wird noch nicht berechnet. `impact_eligible=false` bedeutet, dass für diese Station kein belastbarer automatischer Hydro-Impact abgeleitet werden darf; die Station kann weiterhin angezeigt oder diagnostisch geprüft werden.

`station_basin` allein genügt nicht, weil es nicht zwingend das gesamte oberliegende Einzugsgebiet beschreibt und ohne Upstream-Kette falsche Sicherheit erzeugen kann. Flowline-Snapping ist ebenfalls nur Diagnose: Es beschreibt geometrische Nähe zu einer Gewässerlinie, beweist aber keine Lage oberhalb der Station, keine vollständige Fließverbindung und keine passende Reisezeit. Deshalb bleiben `flow_distance_available=false` und `flow_distance_km=null`, solange keine belastbare gerichtete Fließwegdistanz berechnet wird; `topology_source` und `upstream_source_quality` dokumentieren die Herkunft der heutigen konservativen Zuordnung.

Fehlen statische Hydro-Daten, Upstream-Topologie oder produktive Geometrieunterstützung, bleibt das System lauffähig, meldet Gründe wie `hydro_static_missing` oder `hydro_geometry_unavailable` und erzeugt keinen Hydro-Impact. Das ist bewusst konservativ: Wenn die Upstream-Topologie fehlt, kann das System nicht sicher unterscheiden, ob Niederschlag oberhalb, neben oder unterhalb der Messstelle gefallen ist. Lieber wird kein Impact erzeugt, als eine scheinbar präzise, aber hydrologisch nicht abgesicherte Zuordnung anzuzeigen. Ohne Live-Daten wird Cache/Fallback genutzt oder ein stabiler Fehlerstatus angezeigt; daraus entsteht keine neue fachliche Zuordnung.
...
Erlaubt sind **ausschließlich 12 numerische Stellschrauben** der Vorhersage-Pipeline
(EWMA-Glättung, ML-Gate-Margin, Wind-Blending, Optical-Flow-Schwelle, Bearing-Limits,
Verifikations-Matching) innerhalb harter Bounds. Warnschwellen, Benachrichtigungen,
...
Ab P107 zeigt die Karte „Lokale Analyse am Pi“ zwei Dinge zum autonomen Tuning: die
aktuell wirksamen Werte aller elf einstellbaren Parameter (unabhängig davon, ob
gerade ein Experiment lief) sowie das jeweils letzte Tuning-Ergebnis — den
betroffenen Parameter, den vorgeschlagenen bzw. übernommenen Wert und das Ergebnis
(„übernommen“ bei einer bestätigten Verbesserung, sonst den technischen Ausgang wie
