...
WetterExtended erzeugt Hydro-Impact nur für Stationen mit `impact_eligible=true`. Dieses Flag bedeutet, dass lokale Upstream-Topologie (`upstream_catchment_ids`) vorhanden ist und daraus ein geometrisch vereinigtes oberliegendes Einzugsgebiet (`station_catchment`) erzeugt wurde. `impact_eligible=false` bedeutet, dass für diese Station kein belastbarer automatischer Hydro-Impact abgeleitet werden darf; die Station kann weiterhin angezeigt oder diagnostisch geprüft werden.

`station_basin` allein genügt nicht, weil es nicht zwingend das gesamte oberliegende Einzugsgebiet beschreibt und ohne Upstream-Kette falsche Sicherheit erzeugen kann. Flowline-Snapping ist ebenfalls nur Diagnose: Es beschreibt geometrische Nähe zu einer Gewässerlinie, beweist aber keine Lage oberhalb der Station, keine vollständige Fließverbindung und keine passende Reisezeit.
