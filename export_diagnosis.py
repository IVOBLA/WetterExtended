...
# B357: ConvLSTM-Trainings-Telemetrie (eigene Datei, eigenes Skript
# tools/diagnose_convlstm_training.py).
CONVLSTM_TRAINING_LATEST_NAME = "convlstm_training_diagnosis_latest.json"
CONVLSTM_TRAINING_ERROR_NAME = "convlstm_training_diagnosis_error.json"


def run_convlstm_training_diagnosis_before_export(*, hours: int = 240, base_dir: str | Path | None = None, save_paths: dict | None = None) -> dict:
    """B357: Fasst die ConvLSTM-Trainings-Telemetrie (B356/B357) robust vor jedem
    Export zusammen. Analog run_kinematic_acceleration_diagnosis_before_export():
    Fehler werden persistiert, nicht weitergeworfen -- der Export darf nicht
    blockiert werden. Default-hours deckt > 1 Woche ab (woechentlicher Job).
    """
    base = Path(base_dir or ".").resolve()
    ev = evaluation_dir(base, save_paths)
    ev.mkdir(parents=True, exist_ok=True)
    latest = ev / CONVLSTM_TRAINING_LATEST_NAME
    error = ev / CONVLSTM_TRAINING_ERROR_NAME
    script = Path(__file__).resolve().parent / "tools" / "diagnose_convlstm_training.py"
    cmd = [sys.executable, str(script), "--hours", str(int(hours)), "--base-dir", str(base), "--evaluation-dir", str(ev)]
    try:
        proc = subprocess.run(cmd, cwd=str(base), text=True, capture_output=True, timeout=int(os.getenv("CONVLSTM_TRAINING_DIAGNOSIS_TIMEOUT_S", "45")), check=False)
        if proc.returncode != 0:
            raise RuntimeError((proc.stderr or proc.stdout or f"diagnosis exited rc={proc.returncode}").strip())
        error.unlink(missing_ok=True)
        return {"status": "ok", "path": str(latest), "stdout": proc.stdout[-2000:]}
...
def load_latest_convlstm_training_diagnosis(*, base_dir: str | Path | None = None, save_paths: dict | None = None) -> dict:
    ev = evaluation_dir(base_dir, save_paths)
    for name in (CONVLSTM_TRAINING_LATEST_NAME, CONVLSTM_TRAINING_ERROR_NAME):
        path = ev / name
        if path.exists():
            try:
                data = json.loads(path.read_text(encoding="utf-8"))
                ...
                return {"status": "error", "error": str(exc), "file": name}
    return {"status": "missing", "file": None, "important_findings": [], "recommendations": ["Noch keine ConvLSTM-Trainings-Diagnose ausgefuehrt (B357)."]}
