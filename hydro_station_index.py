...
def build_station_index(stations_geojson: str, basins_geojson: str | None, flowlines_geojson: str | None, output_dir: str | None = None) -> dict:
    ...
    for st in stations.get("features", []):
        ...
        if basin is None and basins:
            ...
            basin = min(basins, key=basin_dist)
            quality = "fallback_nearest_basin"
        flowline = None
        snap_distance = None
        if flowlines:
            candidates = []
        ...
        item = {
            ...
            "catchment_id": basin_id, "upstream_catchment_ids": upstream or [], "catchment_area_km2": round(area, 3),
            "default_lag_min": default_lag, "enabled": quality != "unresolved", "quality": quality,
        }
        index.append(item)
    ...
    _write_json(os.path.join(output_dir, "station_network_index.json"), {"stations": index})
