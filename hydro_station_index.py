...
def _station_admin_enabled(station_id: str) -> bool:
    overrides = hydro_station_overrides.load() or runtime_config.get("HYDRO_STATION_OVERRIDES", getattr(config, "HYDRO_STATION_OVERRIDES", {})) or {}
    ...
    for st in stations.get("features", []):
        ...
        if not pt:
            index.append({
                ...
                "exclusion_reason": "station_point_missing", "reason": ["upstream_catchment_unavailable"], "default_lag_min": default_lag,
            })
            continue
        lon, lat = pt
