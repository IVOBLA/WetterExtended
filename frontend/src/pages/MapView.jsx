...
function _forecastPoints(o, forecast) {
  ...
  if (!isPublicCell(o)) return {}
  const pts = { 0: [o.lat, o.lon] }
  horizons.forEach(h => {
    const la = src?.[`forecast_lat_${h}`]
    const lo = src?.[`forecast_lon_${h}`]
    if (src?.[`forecast_rejected_${h}`] === true) return
    if (la != null && lo != null && Number.isFinite(Number(la)) && Number.isFinite(Number(lo))) pts[h] = [Number(la), Number(lo)]
  })
  return pts
}
