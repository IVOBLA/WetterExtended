...
| B230 | `_utc`-Felder enthielten Lokalzeit: `forecast_created_at_utc` / `target_timestamp_utc` aus lokalen Radar-Frame-Zeiten (Europe/Vienna) korrekt nach echtem UTC konvertiert (Helper `_local_naive_to_utc_iso_z`). Behebt u. a. falsche `forecast_created_in_future` / `verified_before_created`-Ausschluesse in der Diagnose. Dateinamen/Frame-Matching unveraendert | `accuracy_tracker.py`, `tests/test_b230_utc_field_semantics.py` | ✅ erledigt |

| B231 | Richtungsfehler: Mindest-Displacement-Schwelle `KINEMATIC_MIN_INTERVAL_DISP_PX` schliesst Sub-Pixel-Jitter-Intervalle aus der EWMA-Bewegungsableitung aus (Rauschunterdrueckung). Default 0.0 = regressionsneutral; Fallback auf ungefiltert wenn alle Intervalle sub-threshold | `config.py`, `init_runtime_overrides.py`, `prediction.py`, `tests/test_b231_min_interval_disp.py` | ✅ erledigt |
